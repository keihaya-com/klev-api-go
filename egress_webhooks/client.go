...
func (c *Client) Create(ctx context.Context, in CreateParams) (EgressWebhook, error) {
...
func (c *Client) Rotate(ctx context.Context, id EgressWebhookID, expireDuration time.Duration) (EgressWebhook, error) {
	var in = RotateParams{int64(expireDuration.Seconds())}
	return c.RotateRaw(ctx, id, in)
}

func (c *Client) RotateRaw(ctx context.Context, id EgressWebhookID, in RotateParams) (EgressWebhook, error) {
	var out EgressWebhook
	err := c.H.Patch(ctx, fmt.Sprintf("egress_webhook/%s/secret", id), in, &out)
	return out, err
}

func (c *Client) Status(ctx context.Context, id EgressWebhookID) (Status, error) {
	var out Status
